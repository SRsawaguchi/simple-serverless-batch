...
```bash
simple-serverless-batch$ sam build --use-container
```
...
```bash
simple-serverless-batch$ sam logs -n StockCheckerFunction --stack-name simple-serverless-batch --tail
```
...
```bash
simple-serverless-batch$ pip install -r tests/requirements.txt --user
# unit test
simple-serverless-batch$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
simple-serverless-batch$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
